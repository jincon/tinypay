...
## 特点
- 丰富的扩展，支持微信支付（商户直连和服务商）、支付宝支付、银联支付、百度支付、字节跳动支付
- 日志支持，方便调试
...
## 使用文档
- 本代码主要来源于：fengkui，使用文档也可以参考： [https://docs.fengkui.net/pay/](https://docs.fengkui.net/pay/)
...
## 安装
```shell
composer require jincon/tinypay
```
...
$pay = new \jincon\Tinypay\Alipay($alipayConfig); // 支付宝
$pay = new \jincon\Tinypay\Unionpay($unionConfig); // 银联
$pay = new \jincon\Tinypay\Baidu($baiduConfig); // 百度
$pay = new \jincon\Tinypay\Bytedance($bytedanceConfig); // 字节跳动
```
...
class Payment
{
    ...
    protected static function config($type='')
    {
        ...
        $type && self::$pay =(new \jincon\Tinypay())::$type(self::$config);
    }
