...
class Alipay
{
    ...
    private static $config = array(
        ...
        'log_path'      => './logs/alipay.log', // 日志文件路径
    );
    ...
    private function validateOrder($order, $requiredFields = ['order_sn', 'total_amount', 'body'], $message = "订单数组信息缺失")
    {
    ...
    public static function face($order){
        ...
        $params = self::unifiedOrder($order, $params);
        $response = \jincon\Tinypay::post(self::$gateway . '?charset=' . self::$charset, $params);
    ...
    public static function xcx($order){
        $config = self::$config;
        $this->validateOrder($order,['order_sn','total_amount','buyer_id','buyer_open_id']);
    ...
    {
        $config = self::$config;
        $this->validateOrder($order,['refund_sn','refund_amount','order_sn','trade_no']);
