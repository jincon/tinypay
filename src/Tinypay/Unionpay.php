...
class Unionpay
{
    ...
    private static $config = array(
        ...
        'log_path'      => './logs/unionpay.log', // 日志文件路径
    );
    ...
    private function validateOrder($order, $requiredFields = ['order_sn', 'total_amount', 'body'])
    {
        foreach ($requiredFields as $field) {
            if (empty($order[$field])) {
                throw new \InvalidArgumentException("订单数组信息缺失：{$field}");
            }
        }
    }
    ...
    public static function scan($order, $type=false){
        ...
        $params = self::unifiedOrder($order);
        $response = \jincon\Tinypay::post(self::$gateway . '/queryTrans.do', $params);
    ...
    public static function query($order) {
        $this->validateOrder($order,['order_sn','txn_time']);
    ...
    public static function refund($order, $type=false) {
        $config = self::$config;
        $this->validateOrder($order,['refund_sn','query_id']);
        ...
        $params = self::unifiedOrder($order);
        $response = \jincon\Tinypay::post(self::$gateway . '/backTransReq.do', $params);
        parse_str($response, $result);
        unset($result['signPubKeyCert']);
        unset($result['signature']);
        return $result;
