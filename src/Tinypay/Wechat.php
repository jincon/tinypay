 ...
 * @Last Modified time: 2028-08-10 09:26:13
 */
namespace jincon\Tinypay;
...
class Wechat
{
    ...
    private static function validateOrder($order, $requiredFields = ['order_sn', 'total_amount', 'body'], $message = "订单数组信息缺失")
    {
    ...
    public static function unifiedOrder($order, $type=false)
    {
        ...
        $response = \jincon\Tinypay::post($url, json_encode($params, JSON_UNESCAPED_UNICODE), $header);
    ...
    public static function query($orderSn, $type = false)
    {
        ...
        $response = \jincon\Tinypay::get($url, $params, $header);
        $result = json_decode($response, true);
     ...
     * @return [type]        [description]
     */
    public static function jsapi($order=[]){
        $config = self::$config;
        ...
        if (empty($code)) {
            ...
            $response = \jincon\Tinypay::get(self::$accessTokenUrl, $params); // 进行GET请求
            $result = json_decode($response, true);
    ...
    {
        self::validateOrder($order, ['order_sn', 'total_amount', 'body']);
    ...
    public static function h5($order=[], $log=false)
    {
        ...
        if(!in_array($order['type'], ['ios', 'android', 'wap'])){
            throw new \InvalidArgumentException("订单数组type参数错误！");
        }
    ...
    {
        self::validateOrder($order, ['order_sn', 'total_amount', 'body', 'openid']);
    ...
    public static function refund($order)
    {
        ...
        if(empty($order['refund_sn']) || empty($order['refund_amount']) || (empty($order['order_sn']) && empty($order['transaction_id']))){
            throw new \InvalidArgumentException("订单数组信息缺失！");
        }
        $params = array(
    ...
    public static function transfer($order = [])
    {
        ...
        if ($order['amount'] >= 200000 && empty($order['name']))
            throw new \InvalidArgumentException("单笔金额大于两千，请填写用户姓名");
    ...
    {
        self::validateOrder($order, ['transaction_id', 'order_sn'],"转账单号缺失");
    ...
    public static function profitsharingQuery($order = [])
    {
        if (is_array($order) && (empty($order['transaction_id']) || empty($order['order_sn']))) {
            throw new \InvalidArgumentException("支付单号缺失！");
     ...
     */
    // 生成v3 Authorization
    public static function createAuthorization($url, $params=[], $method='POST') {
        $config = self::$config;
        // 商户号（服务商模式使用服务商商户号）
        ...
        $body = [
            ...
            'data'  => is_array($params) ? json_encode($params, JSON_UNESCAPED_UNICODE) : $params, // POST/PUT/DELETE请求时 需要 转JSON字符串
        ];
        ...
        foreach ($result['data'] as $key => $certificate) {
            if (strtotime($certificate['expire_time']) > time()) {
                ...
                if ($publicKey) { // 生成public_key证书文件
                    ...
                    return $type ? $publicKey : $certificate['serial_no']; // 返回证书信息
                    break; // 终止循环
                } else {
